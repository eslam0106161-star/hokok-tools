...
async function uploadLeavesContent(subjectId, leaves) {
  ...
  for (const leaf of leaves) {
    ...
    // فحص دفاعي: ملفات docx مولّدة بنسخة أقدم من generate-book.js (قبل إضافة
    // التعقيم) ممكن يكون فيها "[object Object]" حرفيًا مكان سؤال مقالي. نكتشفها
    // هنا بدل ما تترفع صامتة لقاعدة البيانات.
    const corrupted = [];
    if (/\[object Object\]/.test(parsed.explanation)) corrupted.push('الشرح');
    parsed.mcq.forEach((q, i) => { if (/\[object Object\]/.test(q.text)) corrupted.push(`mcq[${i}]`); });
    parsed.essay.forEach((q, i) => { if (/\[object Object\]/.test(q)) corrupted.push(`essay[${i}]`); });
    if (corrupted.length) {
      corruptionWarnings++;
      console.warn(`  ⚠️  "${leaf.name}": محتوى تالف محتمل (${corrupted.join(', ')}) — على الأرجح ناتج عن نسخة قديمة من generate-book.js. راجع الملف يدويًا قبل الاعتماد على هذا الجزء.`);
    }
  ...
  await flush(true);
  return { filled, totalMcq, totalEssay, corruptionWarnings };
}
...
async function processOneDocx(filePath) {
  ...
  // فحص دفاعي: بعض أدوات الضغط على ويندوز بتحوّل أسماء الملفات العربية لصيغة
  // معطوبة زي "#U0627#U0644..." بدل الحروف العربية الفعلية. لو حصل ده واسم
  // المادة (اللي هيظهر للطالب في التطبيق) مأخوذ من اسم الملف مباشرة، هيترفع
  // اسم غير مفهوم لقاعدة البيانات بصمت. نحذّر بقوة بدل الرفع الصامت.
  if (!SUBJECT_NAME_OVERRIDE && /#U[0-9A-Fa-f]{4}/.test(baseName)) {
    console.warn(`⚠️  تحذير: اسم الملف "${baseName}" يبدو معطوبًا (ترميز خاطئ لأحرف عربية، على الأرجح بسبب أداة ضغط الملف). اسم المادة اللي هيظهر للطالب هيبقى بنفس الشكل المعطوب. الحل: أعد تسمية الملف بالعربي الصحيح قبل الرفع، أو استخدم SUBJECT_NAME_OVERRIDE.`);
  }
  ...
  if (result.corruptionWarnings > 0) {
    console.warn(`⚠️  تنبيه: ${result.corruptionWarnings} جزء فيه محتوى تالف محتمل (راجع التحذيرات أعلاه) — تم رفعه لقاعدة البيانات كما هو، يُنصح بمراجعته يدويًا أو إعادة توليد الكتاب بنسخة أحدث من generate-book.js.`);
  }
}
